...
{
    public static List<GridTile> GetTilesInRangeFromTile(GridTile originTile, int range, bool includeOriginTile){
        ...
        if(!includeOriginTile)  inRangeTiles.Remove(originTile);
        return inRangeTiles.Distinct().ToList();
    }

    public static List<GridTile> GetWalkableTilesInRange(GridTile originTile, int range, bool includeOriginTile){
        List<GridTile> inRangeTiles = new();
        ...
        for (int i = 0; i < stepCount; i++)
        {
            ...
            foreach (GridTile openTile in openSetCopy)
            {
                ...
                foreach (GridTile adjacency in GridManager.GetAdjacentTiles(openTile.gridPosition))
                {
                    ...
                    if(GridManager.GetTilesOccupyingObject(adjacency.gridPosition) != null) continue;
                    openSet.Add(adjacency);
                }
            }
        }
    ...
    public static List<GridTile> GetAttackableTilesInRange(GridTile originTile, int range, bool includeOriginTile){
        List<GridTile> inRangeTiles = new();
